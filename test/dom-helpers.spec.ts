...
describe("DOM", () => {
  ...
  describe("getSelector", () => {
    ...
    // TODO: support returning partial aria names
    it.skip("should return an aria name like selector for the closest link or button if the text is not an exact match", async () => {
      await page.setContent(
    ...
    it.skip("should pierce shadow roots to get an aria name", async () => {
      await page.setContent(
        `
        <script>
      ...
      expect(selector).toBe('aria/link[name="Hello World"]');
    });
  });
});
